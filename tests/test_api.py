...
def test_create_weather_data(client):
    ...
    response = client.post('/devgrid/weather', json={'user_id': user_id, 'city_ids': city_ids_list})
    assert response.status_code == 200
    data = response.json()
    assert 'request_info' in data
    assert 'data' in data

def test_get_weather_progress(client):
    global user_id
    assert user_id is not None, "user_id is not set. Run test_create_weather_data first."
    response = client.get(f'/devgrid/weather/{user_id}')
    assert response.status_code == 200
    data = response.json()
    assert 'progress_percentage' in data

def test_get_weather_post_info(client):
    global user_id
    assert user_id is not None, "user_id is not set. Run test_create_weather_data first."
    response = client.get(f'/devgrid/weather_post_info/{user_id}')
    assert response.status_code == 200
